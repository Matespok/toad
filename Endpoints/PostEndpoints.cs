...
public record CreatePostDto(string topic, string content);
...
public static class PostEndpoints
{
    public static void MapPostEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        group
            .MapPost(
                "/",
                async (ClaimsPrincipal user, CreatePostDto model, IForumRepository repo) =>
                {
                    try
                    {
                        var userIdClaim =
                        ...
                        await repo.AddPostAsync(authenticatedUserId, model.topic, model.content);
