...
public static class CommentEndpoints
{
    public static void MapCommentEndpoints(this IEndpointRouteBuilder endpoints) 
    {
        ...
        group.MapPost("/", async (ClaimsPrincipal user, int postId, int? parentId, string content, IForumRepository repo) =>
            {
                try
                {
            ...
            .WithDescription("Adds a new comment to a specific post. Requires postId and content. Requires valid JWT token.");
        ...
        .WithDescription("Fetches a list of all comments associated with the specified post ID.");
    
        // API GET - Comments for user
        group.MapGet("/users/{id}", async (int id, IForumRepository repo) =>
        {
            try
            {
                var comments = await repo.GetPostsByUserIdAsync(id);
                return comments is not null ? Results.Ok(comments) : Results.NoContent();
            }
            catch (Exception e)
            {
